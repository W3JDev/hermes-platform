...
async def lifespan(app: FastAPI):
    # init handled in bootstrap; this is just a sanity check
    yield
...
def list_vars(request: Request, scope: Optional[str] = None):
    ...
    for r in rows:
        try:
            value = fernet.decrypt(r["value_encrypted"]).decode("utf-8")
        except Exception:
            value = ""
        out.append({
            "name": r["name"],
...
def get_var(name: str, request: Request):
    ...
    fernet = _get_fernet()
    value = fernet.decrypt(row["value_encrypted"]).decode("utf-8")
